...
export function SearchModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Search messages" className="max-w-2xl">
      ...
      <div className="mt-4 max-h-[min(28rem,55vh)] overflow-y-auto">
              ...
              {hits.map((h, i) => (
                <li key={h.message_id}>
                  <button
                    ...
                    onClick={() => goto(h)}
                    className={cn(
                      "flex w-full gap-3 rounded-[--r-md] px-3 py-2.5 text-left transition-colors duration-[--d-micro]",
                      i === cursor ? "bg-[--bg-hover]" : "hover:bg-[--bg-hover]",
                    )}
                  >
                    <span className="mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-[--r-sm] bg-[--bg-active] text-[--fg-muted]">
                      {h.sender_type === "ai" ? (
