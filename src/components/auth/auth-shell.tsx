    ...
    <div className="relative flex min-h-dvh flex-col overflow-hidden bg-[--bg]">
      ...
      <header className="flex items-center justify-between px-5 py-5">
        <Link href="/" className="group flex items-center gap-2.5">
          <Logo className="h-7 w-7 transition-transform duration-[--d-micro] group-hover:scale-105" />
      ...
      <main className="flex flex-1 items-center justify-center px-5 pb-16">
        ...
        >
          <motion.div variants={riseIn} className="mb-7 text-center">
            ...
            {subtitle && (
              <p className="mt-2 text-[14px] leading-relaxed text-[--fg-muted]">{subtitle}</p>
            )}
          </motion.div>

          <motion.div
            variants={riseIn}
            className="overflow-hidden rounded-[--r-lg] border border-[--border]/70 bg-[--surface]/90 p-6 shadow-[0_1px_0_0_rgba(255,255,255,0.05)_inset,var(--e2)] backdrop-blur-xl supports-[backdrop-filter]:bg-[--surface]/75"
          >
            {DEMO_MODE && (
